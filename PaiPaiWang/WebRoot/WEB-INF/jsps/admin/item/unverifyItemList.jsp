...
<html>
	...
	<body>
		...
		<table border="1px" bordercolor="#cccccc" cellspacing="0px" cellpadding="0px" class="center_table">
			...
			<c:forEach items="${page.pageMap.rows}" var="object">
				<tr>
					...
					<td>${object.owner}</td>
					<td>
						<fmt:formatDate value="${object.publishTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
					</td>
					<td>
					...
					<td>${object.price}</td>
					<td>
						<a href="${path}/item/admin_verifyItemDetail?id=${object.id}">审核</a>	
					</td>
				</tr>
			</c:forEach>
		</table>
		...
		<div class="page_bar">
			<a href="${path}/item/admin_getUnverifyItemPage?page=${requestScope.page.lastPage}&keyWord=${requestScope.page.keyWord}">上一页</a>
			<span>第${currentPage}页/共${totalPage}页</span>
			<a href="${path}/item/admin_getUnverifyItemPage?page=${requestScope.page.nextPage}&keyWord=${requestScope.page.keyWord}">下一页</a>
		</div>
		...
		<script type="text/javascript">
			...
			(function(){
				...
				$("#search_btn").click(function(){
					...
					//只有当输入框中不为空的时候才进行搜索
					if(keyWord != ""){
						window.location.href="${path}/item/admin_getUnverifyItemPage?keyWord="+keyWord;
					}
				});
