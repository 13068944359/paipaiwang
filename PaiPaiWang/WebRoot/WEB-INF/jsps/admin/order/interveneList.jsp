	...
	<body>
		<div class="location">
			...
			<div class="search_bar">
				<input id="search_key_word" type="text" name="search_key_word" placeholder="按订单号查找"><input id="search_btn" type="submit" value="搜索">
			</div>
		</div>
		...
		<table border="1px" bordercolor="#cccccc" cellspacing="0px" cellpadding="0px" class="center_table">
			...
			<c:forEach items="${page.pageMap.rows}" var="object">
				<tr>
					...
					<td>${object.requestTime}</td>
					
					<c:if test="${object.handled == true}">
						<td>已处理</td>		
					</c:if>
					<c:if test="${object.handled != true}">
						<td>
							<a href="${path}/order/admin_interveneDetailPage?id=${object.interveneId}">介入处理</a>
						</td>
					</c:if>
				</tr>
			</c:forEach>
		...
		<div class="page_bar">
			<a href="${path}/order/admin_getInterveneListPage?page=${requestScope.page.lastPage}&keyWord=${requestScope.page.keyWord}">上一页</a>
			<span>第${currentPage}页/共${totalPage}页</span>
			<a href="${path}/order/admin_getInterveneListPage?page=${requestScope.page.nextPage}&keyWord=${requestScope.page.keyWord}">下一页</a>
		</div>
		...
		<script type="text/javascript">
			...
			(function(){
				...
				$("#search_btn").click(function(){
					...
					//只有当输入框中不为空的时候才进行搜索
					if(keyWord != ""){
						window.location.href="${path}/order/admin_getInterveneListPage?keyWord="+keyWord;
					}
				});
