	...
	<body>
		...
		<table border="1px" bordercolor="#cccccc" cellspacing="0px" cellpadding="0px" class="center_table">
   			<tr>
				...
				<td>${interveneVO.orderDetailVO.id}</td>
			</tr>
			<tr>
				<td>卖家</td>
				<td>${interveneVO.orderDetailVO.seller}</td>
			</tr>
			<tr>
				<td>买家</td>
				<td>${interveneVO.orderDetailVO.buyer}</td>
			</tr>
			<tr>
				<td>价格</td>
				<td>${interveneVO.orderDetailVO.price}</td>
			</tr>
			<tr>
				<td>生成日期</td>
				<td>
					<fmt:formatDate value="${interveneVO.orderDetailVO.createDate}" pattern="yyyy-MM-dd"/>
				</td>
			</tr>
			<tr>
				<td>订单状态</td>
				<td>${interveneVO.orderDetailVO.state}</td>
			</tr>
			<c:if test="${interveneVO.orderDetailVO.finishTime!=null}">
				<tr>
					<td>完成交易时间</td>
					<td>
						<fmt:formatDate value="${interveneVO.orderDetailVO.finishTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
					</td>
				</tr>
			</c:if>
			
			<c:if test="${interveneVO.orderDetailVO.sendName!=null}">
				<tr>
					<td>收货人姓名</td>
					<td>${interveneVO.orderDetailVO.sendName}</td>
				</tr>
				<tr>
					<td>收货人电话号码</td>
					<td>${interveneVO.orderDetailVO.sendPhone}</td>
				</tr>
				<tr>
					<td>收货人地址</td>
					<td>${interveneVO.orderDetailVO.sendAddress}</td>
				</tr>
				<tr>
					<td>收货人邮政编码</td>
					<td>${interveneVO.orderDetailVO.sendPostcode}</td>
				</tr>
				<tr>
					<td>收货备注信息</td>
					<td>${interveneVO.orderDetailVO.sendOthers}</td>
				</tr>
			</c:if>
			
			<c:if test="${interveneVO.orderDetailVO.expressCompany!=null}">
				<tr>
					<td>快递公司</td>
					<td>${interveneVO.orderDetailVO.expressCompany}</td>
				</tr>
				<tr>
					<td>快递单号</td>
					<td>${interveneVO.orderDetailVO.expressCode}</td>
				</tr>
			</c:if>
			
			<c:if test="${interveneVO.orderDetailVO.evaluation!=null}">
				<tr>
					<td>用户评价</td>
					<td>${interveneVO.orderDetailVO.evaluation}</td>
				</tr>
			</c:if>
			
			<c:if test="${interveneVO.orderDetailVO.returnReason!=null}">
				<tr>
					<td>退货理由描述</td>
					<td>${interveneVO.orderDetailVO.returnReason}</td>
				</tr>
			</c:if>
			<tr>
				<td  style="color:red;">当前订单冻结总金额</td>
				<td>${interveneVO.freezeMoney}</td>
			</tr>
			<tr>
				<td  style="color:red;">申请诉求</td>
				<td>${interveneVO.forWhat}</td>
			</tr>
			<tr>
				<td  style="color:red;">申请原因</td>
				<td>${interveneVO.forWhy}</td>
			</tr>
			<tr>
				<td colspan="2">
					<a href="${path}/order/admin_handleIntervene?id=${interveneVO.interveneId}&agree=true">介入处理（收缴冻结资金）</a>&nbsp;&nbsp;&nbsp;&nbsp;
					<a href="${path}/order/admin_handleIntervene?id=${interveneVO.interveneId}&agree=false">否决申请</a>
				</td>
			</tr>
		</table>
